    ...
    validate({action}: SelectData) {
        ...
        if (cost !== undefined && cost > 0) {
            assert(this.currentPlayer().money >= cost, {
                invalidInput: "Not enough money to pay the cost for the selected action."
            });
        }
    }
...
export const GoldsmithVariant = new Key(
    "GoldsmithVariant",
    z.number(),
);
export const GOLDSMITH_VARIANT_NO_MINING_EXPERTISE = 1;
export const GOLDSMITH_VARIANT_BONUS_INCOME = 2;
export const PickGoldsmithVariantData = z.object({ goldsmithVariant: z.number()});
export type PickGoldsmithVariantData = z.infer<typeof PickGoldsmithVariantData>;

export class MinasGeraesPickGoldsmithVariantAction implements ActionProcessor<PickGoldsmithVariantData> {
    ...
    validate(data: PickGoldsmithVariantData) {
        ...
        assert(data.goldsmithVariant === GOLDSMITH_VARIANT_NO_MINING_EXPERTISE || data.goldsmithVariant == GOLDSMITH_VARIANT_BONUS_INCOME);
    }
    ...
    process(data: PickGoldsmithVariantData): boolean {
        ...
        if (data.goldsmithVariant === GOLDSMITH_VARIANT_NO_MINING_EXPERTISE) {
            this.log.currentPlayer("chooses to use the Goldsmith action to not spend mining expertise when delivering gold.")
        } else if (data.goldsmithVariant === GOLDSMITH_VARIANT_BONUS_INCOME) {
            this.log.currentPlayer("chooses to use the Goldsmith action to receive an additional income when delivering gold this round.")
        }
        return true;
    }
}
