...
import {ChesapeakeAndOhioMapData} from "../chesapeake-and-ohio/build";
import {coordinatesToCenter} from "../../utils/point";
import * as factoryStyles from "../chesapeake-and-ohio/rivers.module.css";
...
export function MinasGeraesOverlayLayer(props: TexturesProps) {
    ...
    for (const [_, space] of props.grid.entries()) {
        ...
        if (mapData && mapData.miningTown) {
            ...
            result.push(
                <circle cx={center.x} cy={center.y} fill="black" r={props.size * 0.4} />
            );
        }
    }
    return result;
